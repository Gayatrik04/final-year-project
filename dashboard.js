...
async function fetchTransactions() {
  try {
    const res = await fetch(`http://localhost:5000/transactions/${currentUser}`);
...
function updateSummary(transactions) {
  ...
  if (Object.keys(categoryTotals).length > 0) {
    highestCategory = Object.entries(categoryTotals).sort((a, b) => b[1] - a[1])[0][0];
  }

  // Most Frequent Category
  let mostFreqCategory = "-";
  if (Object.keys(categoryCounts).length > 0) {
    mostFreqCategory = Object.entries(categoryCounts).sort((a, b) => b[1] - a[1])[0][0];
  }
  ...
  document.getElementById("avgMonthlySpending").textContent = `₹${avgMonthlySpending.toFixed(2)}`;
}
...
function populateTables(transactions) {
  ...
  transactions.slice(0, 5).forEach(tx => {
    recentTable.innerHTML += `
      <tr>
        ...
        <td>${tx.category === "Income" ? "+" : "-"}₹${Number(tx.amount).toFixed(2)}</td>
      </tr>`;
  });

  // Top 5 Expenses
  const expenses = transactions.filter(tx => tx.category !== "Income").sort((a, b) => b.amount - a.amount).slice(0, 5);
  ...
  // Top 5 Incomes
  const incomes = transactions.filter(tx => tx.category === "Income").sort((a, b) => b.amount - a.amount).slice(0, 5);
